...
import { runStrategy, type RSIPoint } from '@/lib/rsi';
...
export default function GoldChart({ data, lastFetchTime }: GoldChartProps) {
  ...
  useEffect(() => {
    ...
    if (!candleSeriesRef.current || hasSetInitialData.current) return;
  ...
  useEffect(() => {
    if (data.length === 0) return;
    if (!candleSeriesRef.current || !hasSetInitialData.current) return;
    // Only update if data length changed (new candle) or last candle updated
    ...
    // Update RSI if we have enough new data
